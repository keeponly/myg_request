...
test_data = DoExcel(project_path.case_path, 'underLineReportAuditing').read_excel('underLineReportAuditingCASE')
My_log = Mylog()
token = None
@ddt
class test_p3_underLineReportAuditing(unittest.TestCase):
    def setUp(self):
        self.T = DoExcel(project_path.case_path, 'underLineReportAuditing')
    def tearDown(self):
        pass
    @data(*test_data)
    def test_p3_underLineReportAuditing(self, case):
        '''附件审核，初审人通过'''
        ...
        if "%" in param1:
            ...
            taskId1 = getattr(GetData, "taskId1")
            print("projectId:{},investorId:{},taskId1{}".format(projectId,investorId,taskId1))
            param1 = param1 % ({"projectId": projectId,"investorId":investorId,"taskId1":taskId1})
