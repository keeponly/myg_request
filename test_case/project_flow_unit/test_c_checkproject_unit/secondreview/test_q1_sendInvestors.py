...
test_data = DoExcel(project_path.case_path, 'sendInvestors').read_excel('sendInvestorsCASE1')
My_log = Mylog()
token = None
@ddt
class test_q1_sendInvestors(unittest.TestCase):
    def setUp(self):
        self.T = DoExcel(project_path.case_path, 'sendInvestors')
    def tearDown(self):
        pass
    @data(*test_data)
    def test_q1_testingedu_auth(self, case):
        '''拒绝之后再次送审'''
