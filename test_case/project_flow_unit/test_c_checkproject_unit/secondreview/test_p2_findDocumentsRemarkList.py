...
token = None
@ddt
class test_p2_findDocumentsRemarkList(unittest.TestCase):
    def setUp(self):
        self.T = DoExcel(project_path.case_path, 'findDocumentsRemarkList')
    def tearDown(self):
        pass
    @data(*test_data)
    def test_p2_findDocumentsRemarkList(self, case):
        '''获取公司的taskid'''
        ...
        if case['CaseId']==1:
            #公司初审的taskid
            taskid1value = resp.json()["data"]["taskId"]
            setattr(GetData, "taskId1", taskid1value)
            print('taskId1的值是>>>>>>>>>>' + taskid1value)
