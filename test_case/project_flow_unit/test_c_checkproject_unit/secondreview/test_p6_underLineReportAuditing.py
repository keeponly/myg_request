...
test_data = DoExcel(project_path.case_path, 'underLineReportAuditing').read_excel('underLineReportAuditingCASE1')
My_log = Mylog()
token = None
@ddt
class test_p6_underLineReportAuditing(unittest.TestCase):
    def setUp(self):
        self.T = DoExcel(project_path.case_path, 'underLineReportAuditing')
    def tearDown(self):
        pass
    @data(*test_data)
    def test_p6_underLineReportAuditing(self, case):
        '''附件审核，初审人拒绝'''
        ...
        if "%" in param1:
            ...
            taskId2 = getattr(GetData, "taskId2")
            print("projectId:{},investorId:{},taskId2{}".format(projectId,investorId,taskId2))
            param1 = param1 % ({"projectId": projectId,"investorId":investorId,"taskId2":taskId2})
