...
test_data = DoExcel(project_path.case_path, 'repeatAuditRequest').read_excel('repeatAuditRequestCASE')
My_log = Mylog()
@ddt
class test_o4_repeatAuditRequest(unittest.TestCase):
    def setUp(self):
        self.T = DoExcel(project_path.case_path, 'repeatAuditRequest')
    ...
    # 发起测试
        if "%" in param1:
            ...
            print("projectId:{},projectNumber:{},taskId:{},originalInvestorId:{}".format(projectId,projectNumber,taskId,originalInvestorId))
            param1 = param1 % ({"projectId": projectId,"projectNumber":projectNumber,"taskId":taskId,"originalInvestorId":originalInvestorId})
