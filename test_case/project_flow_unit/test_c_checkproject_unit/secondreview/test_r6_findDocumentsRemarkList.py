...
test_data = DoExcel(project_path.case_path, 'findDocumentsRemarkList').read_excel('findDocumentsRemarkListCASE')
My_log = Mylog()
token = None
@ddt
class test_r2_findDocumentsRemarkList(unittest.TestCase):
    def setUp(self):
        self.T = DoExcel(project_path.case_path, 'findDocumentsRemarkList')
    def tearDown(self):
        pass
    @data(*test_data)
    def test_r2_findDocumentsRemarkList(self, case):
        '''获取签发通过的taskid'''
        ...
        headers = dict(token=getattr(GetData, "token_qianfa"))
        ...
        if case['CaseId']==1:
            #公司签发通过的taskid
            taskId6value = resp.json()["data"]["taskId"]
            setattr(GetData, "taskId6", taskId6value)
            print('taskId6的值是>>>>>>>>>>' + taskId6value)
