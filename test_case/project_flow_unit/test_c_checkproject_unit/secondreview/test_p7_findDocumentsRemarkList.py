...
token = None
@ddt
class test_p7_findDocumentsRemarkList(unittest.TestCase):
    def setUp(self):
        self.T = DoExcel(project_path.case_path, 'findDocumentsRemarkList')
    def tearDown(self):
        pass
    @data(*test_data)
    def test_p7_findDocumentsRemarkList(self, case):
        '''获取添加意见的的taskid'''
        ...
        if case['CaseId']==3:
            #添加意见的taskID
            taskId3value = resp.json()["data"]["findTaskNodeInfo"][0]["list"][0]["usertask"][0]["taskId"]
            setattr(GetData, "taskId3", taskId3value)
            print('taskId3的值是>>>>>>>>>>' + taskId3value)
            #项目的id
            procInstIdvalue =  resp.json()["data"]["findTaskNodeInfo"][0]["list"][0]["usertask"][0]["procInstId"]
            setattr(GetData, "procInstId", procInstIdvalue)
            print('procInstId的值>>>>>>>>>>' + str(procInstIdvalue))
